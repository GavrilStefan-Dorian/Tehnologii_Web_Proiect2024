...
async function getBook(id, user = null)
{
    let books = [];
    if(!user)
        books = await sql`SELECT books.book_id, books.title, books.author, books.description, books.coverimg, books.liked, books.bookmarked, books.status, AVG(reviews.rating) as boo_rating, COUNT(*) as boo_numratings FROM (SELECT * FROM books WHERE book_id = ${id}) books JOIN reviews ON books.book_id = reviews.book_id
    ...
    END AS bookmarked, CASE 
        ...
        ELSE NULL
    END AS status FROM (
    SELECT books.book_id, books.title, books.author, books.description, books.coverimg, AVG(reviews.rating) as boo_rating, COUNT(*) as boo_numratings FROM (SELECT * FROM books Where book_id = ${id}) books
    JOIN reviews ON books.book_id = reviews.book_id GROUP BY books.book_id, books.title, books.author, books.description, books.coverimg) books
...
{
    return sql`SELECT books.book_id, books.title, books.author, books.description, books.coverimg, books.status, AVG(reviews.rating) as boo_rating, COUNT(*) as boo_numratings FROM (SELECT books.book_id, books.title, books.author, books.description, books.coverimg, status FROM book_reading_status JOIN books ON books.book_id = book_reading_status.book_id WHERE book_reading_status.user_id = ${user_id}) books
                JOIN reviews ON books.book_id = reviews.book_id GROUP BY books.book_id, books.title, books.author, books.description, books.coverimg, books.status;`;
}
